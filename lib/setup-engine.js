...
class SetupEngine {
    ...
    async initContent() {
        ...
        const HTML_FILES = [
            'index.html', 'rules.html', 'rules-overview.html', 'rules-guide.html',
            'rules-glossary.html', 'dashboard.html', 'guides.html', 'how-to-join.html',
            'staff.html', 'city-guide.html', 'economy-guide.html', 'recruits-guide.html',
        ];
        ...
        try {
            ...
            for (const filename of HTML_FILES) {
                ...
                if (!fs.existsSync(filePath)) {
                    this.emit('warn', `Файл не найден: ${filename}`);
                    continue;
                }
                ...
                if (existing.rows.length > 0 && this.options.isFresh) {
                    await client.query('DELETE FROM page_content WHERE page_name = $1', [pageName]);
                }
                ...
                loaded++;
                this.emit('info', `Загружена страница: ${pageName}`);
            }
